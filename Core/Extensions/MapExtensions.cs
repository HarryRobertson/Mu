...
{
    public static MuApplication Map(this MuApplication app, string eventType, Func<MuContext, Func<Task>, Task> handler)
    {
        var predicatedHandler = (MuContext context, Func<Task> next) 
            => context.ConsumedEvent.Headers is Headers headers && headers.EventType.ToLower() == eventType.ToLower()
